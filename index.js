...
bot.setMyCommands([
  ...
  { command: 'fortune', description: 'Гадание на имени' },
  { command: 'help', description: 'Список всех команд' },
  { command: 'status', description: 'Статус бота (владелец)' },
  { command: 'online', description: 'Режим на связи (владелец)' },
  { command: 'offline', description: 'Режим авто-ответа (владелец)' }
]);
...
"```";
  ...
  for (let i = 1; i < frames.length; i++) {
    ...
    try {
      await bot.editMessageText(frames[i], { chat_id: chatId, message_id: sent.message_id, parse_mode: "Markdown" });
    } catch (e) {}
  }
});
